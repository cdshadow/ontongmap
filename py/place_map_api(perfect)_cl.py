# place_map_api_v4(clear_지오코딩 + 현위치 버튼)
#
# 기존 place_map_api(perfect).py 와 동일하게 주소를 좌표로 변환하여
# 지도를 만들되, 아래 세 가지를 추가하여 index.html 과 같은 결과물을 만든다.
#
#   (1) 모바일 전체화면 대응 뷰포트  : viewport-fit=cover
#   (2) 화면 우측 상단 "현위치" 버튼  : HTML + CSS
#   (3) 버튼 동작용 Geolocation 스크립트
#
# 저장 파일명은 facility_map.html 이 아니라 index.html 이다.
...
file_path = base_dir / "place_map_template.csv"
...
marker_count = int((data["x"].notnull() & data["y"].notnull()).sum())
failed_count = total_count - marker_count

print(f"전체 {total_count}건 중 {marker_count}건 표시, {failed_count}건 좌표 변환 실패")

if failed_count:
    failed_names = data.loc[
        data["x"].isnull() | data["y"].isnull(),
        "name"
    ].tolist()

    print(f"좌표 변환 실패 목록: {failed_names}")
