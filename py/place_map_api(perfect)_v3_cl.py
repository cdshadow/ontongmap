...
#
# 두 종류의 마커는 모두 반투명하게 그려 겹친 지점의 지도가 비쳐 보이게 한다.
# 주유소 마커에는 휘발유 가격을 항상 보이는 라벨로 붙인다.
# 가격 라벨은 읽기 쉽도록 반투명 처리 대상에서 제외한다.
...
# 마커 투명도
# 1.0이 불투명이고 값이 작을수록 투명해진다.
# 마커가 겹쳐 있어도 아래쪽 마커와 지도가 비쳐 보이도록 반투명하게 둔다.
# 이 값은 마커 아이콘에만 적용되며 주유소 가격 라벨에는 영향을 주지 않는다.
