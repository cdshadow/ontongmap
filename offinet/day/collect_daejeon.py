...
def build(raw_dir, ymd):
    ...
    # 교차검증: 주소에서 뽑은 구가 파일의 구와 맞는지
    # ("대전 광역시 중구"처럼 띄어쓰기가 다른 표기도 잡는다)
    addr_gu = df["주소"].str.extract(r"대전(?:\s*광역시)?\s+(\S+구)")[0]
    mism = df[addr_gu.notna() & (addr_gu != df["구"])]
    bad_files = sorted(mism["구"].unique().tolist())
    ...
    return (df.sort_values(["구", "휘발유"], na_position="last")
              .reset_index(drop=True), counts, bad_files)
...
def main():
    ...
    df, counts, bad_files = build(raw_dir, ymd)
    if df is None:
    ...
    if bad_files:
        problems.append(
            f"요청한 구와 다른 지역 데이터가 섞임: {bad_files} — "
            f"서버가 구 필터를 무시했을 가능성이 있습니다.")
