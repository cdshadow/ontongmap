# place_map_api_v5(온통대전 가맹점 + 주유소 휘발유 가격)
#
# 두 개의 CSV를 함께 읽어 하나의 index.html을 만든다.
#
#   place_map_template.csv        온통대전 가맹점  -> 파란색 마커
#   지역_위치별(주유소)_cl.csv     주유소           -> 빨간색 마커
#
# 온통대전 가맹점은 두꺼운 파란색 테두리에 속이 비어 있는 원으로,
...
ontong_data["구분"] = "온통대전"
ontong_data["휘발유"] = pd.NA

# ------------------------------------------------------------
# 4-2. 주유소 (상호, 주소, 휘발유 / utf-8-sig)
# ------------------------------------------------------------

station_path = base_dir / "지역_위치별(주유소)_cl.csv"
...
# 온통대전 가맹점 원의 모양
# 테두리는 진한 파란색으로 두껍게 그리고, 안쪽은 채우지 않는다.
# fill_opacity를 0으로 두면 원 안으로 지도가 그대로 보인다.
...
for index, row in data.iterrows():
    ...
    # --------------------------------------------------------
    # 8-1. 온통대전 가맹점 : 두꺼운 파란색 테두리에 속이 빈 원
    # --------------------------------------------------------
    ...
    if category == "온통대전":
        ...
        folium.CircleMarker(
            location=location,
            radius=ontong_radius,
            popup=place_name,
            tooltip=place_name,

            # 테두리는 진하게, 안쪽은 비워 둔다.
            color=ontong_color,
            weight=ontong_weight,
            opacity=1.0,
            fill=True,
            fill_opacity=0.0
        ).add_to(facility_map)
    ...
    # --------------------------------------------------------
        ...
        folium.Marker(
            location=location,
            popup=f"{place_name}<br>휘발유 {price_text}",
