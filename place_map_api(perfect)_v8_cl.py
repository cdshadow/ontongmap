# place_map_api_v8(삼각형 가맹점 + 역삼각형 주유소, 겹치면 별 모양)
#
...
#
# 두 마커 모두 배경을 채우지 않고 테두리만 그린다.
#   온통대전 가맹점 : 파란색 삼각형   (꼭짓점이 위)
#   주유소          : 빨간색 역삼각형 (꼭짓점이 아래)
#
# 같은 자리에 두 마커가 함께 놓이면 육각별(✡) 모양이 되도록
# 파란 삼각형은 조금 위로, 빨간 역삼각형은 조금 아래로 그린다.
...
# 마커의 모양
#
# 두 마커 모두 색이 채워진 기본 아이콘 대신
# 테두리만 그린 SVG를 직접 만들어 쓴다.
# fill="none" 이므로 도형 안쪽으로 지도가 그대로 보인다.
#
#   온통대전 가맹점 : 파란색 삼각형   (▲)
#   주유소          : 빨간색 역삼각형 (▼)
...
ontong_points = "13,2.5 24,21.5 2,21.5"

# 역삼각형 : 위쪽 밑변과 아래쪽 꼭짓점 하나
station_points = "2,2.5 24,2.5 13,21.5"

ontong_svg = make_triangle_svg(ontong_points, ontong_color)
station_svg = make_triangle_svg(station_points, station_color)

# 마커를 좌표에 맞출 때 도형의 한가운데가 아니라
# 삼각형의 무게중심을 기준으로 삼는다.
#
# 위를 향한 삼각형은 무게중심이 도형 중심보다 아래에 있으므로 조금 위로 올라가고,
# 아래를 향한 역삼각형은 무게중심이 위에 있으므로 조금 아래로 내려간다.
# 두 무게중심이 같은 좌표에 놓이므로 한자리에서 겹치면 육각별 모양이 된다.
...
for index, row in data.iterrows():
    ...
    if category == "온통대전":
        ...
        folium.Marker(
            ...
            # 삼각형의 무게중심이 실제 좌표에 놓이도록 맞춘다.
            icon=folium.DivIcon(
                html=ontong_svg,
                icon_size=(marker_width, marker_height),
                icon_anchor=(marker_width / 2, ontong_anchor_y),
                class_name="shape-marker"
            )
        ).add_to(facility_map)

    # --------------------------------------------------------
    # 8-2. 주유소 : 빨간색 역삼각형(속 비움) + 휘발유 가격 라벨
    # --------------------------------------------------------
        ...
        folium.Marker(
            ...
            icon=folium.DivIcon(
                html=station_svg,
                icon_size=(marker_width, marker_height),
                icon_anchor=(marker_width / 2, station_anchor_y),
                class_name="shape-marker"
            )
        ).add_to(facility_map)
